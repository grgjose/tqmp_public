...
class ProductController extends Controller
{
    ...
    /**
     * Checkout Items of User (OTP Get)
     */
    public function checkout_otp_get(Request $request)
    {
        /** @var \Illuminate\Auth\SessionGuard $auth */
        $auth = auth();
        $my_user = $auth->user();
        ...
        if ($my_user == null) {
            return response()->json([
                'success' => false,
                'message' => 'Something went wrong with the session of the user.'
            ], 401);
        }
    ...
    /**
     * Checkout Items of User (OTP Post)
     */
    public function checkout_otp_post(Request $request)
    {
        ...
        if($auth == null) return redirect('/')->with('error_msg', 'Something went wrong with the session of the user.');
        if($my_user == null) return redirect('/')->with('error_msg', 'Something went wrong with the session of the user.');
    ...
    public function emailTest()
    {
        Mail::raw('Test email', function ($message) {
            $message->to('georgelouisjsoe@gmail.com')->subject('SMTP Test');
        });
