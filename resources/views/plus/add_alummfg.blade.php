<div class="py-5">
    <div class="row">
        <div class="mb-3">
            <input type="text" id="productSearch1" class="form-control form-control-lg" placeholder="Search Aluminum Products...">
        </div>
        <div class="col-12 col-md-3 mb-3 mb-md-0 pe-md-0">
            <div class="nav flex-row flex-md-column nav-pills me-md-3 border-end border-md-2" id="subcategory3-tabs"
                role="tablist" style="border-right-color:#65000B; overflow-x:auto; white-space:nowrap;">
                ...
                @foreach ($productSubCategories as $sub_category)
                    @if ($sub_category->category_id == 3)
                        <button class="nav-link mt-2 me-2 me-md-0 {{ $isFirst ? 'active' : '' }}"
                            id="tab3-{{ $sub_category->id }}" data-bs-toggle="tab"
                            data-bs-target="#pane3-{{ $sub_category->id }}" type="button" role="tab"
                            aria-controls="pane3-{{ $sub_category->id }}"
        ...
        <div class="col-12 col-md-9 tab-content-container">
            <div class="tab-content" id="subcategory3-content">
                @php $isFirst = true; @endphp
                @foreach ($productSubCategories as $sub_category)
                    @if ($sub_category->category_id == 3)
                        <div class="tab-pane fade {{ $isFirst ? 'show active' : '' }}"
                            id="pane3-{{ $sub_category->id }}" role="tabpanel"
                            aria-labelledby="tab3-{{ $sub_category->id }}">
                                                ...
                                                <div class="card border-1 d-flex flex-column w-100">
                                                    ...
                                                    <div class="card-body d-flex flex-column">
                                                        <h6 class="card-title text-start fw-bold">
                                                            {{ $product->display_name }}</h6>
                                                        @if ($my_user->isDiscounted)
                                                            <h6 class="card-text text-start">
                                                                <small
                                                                    class="text-decoration-line-through text-muted">₱{{ $product->price }}</small>
                                                                ₱{{ $product->discounted_price }}
                                                            </h6>
                                                        @else
                                                            <h6 class="card-text text-start">₱{{ $product->price }}
                                                            </h6>
                                                        @endif
