<div class="py-5">
  <div class="row">
        ...
        @foreach($productSubCategories as $sub_category)
          @if($sub_category->category_id == 2)
            <button class="nav-link mt-2 me-2 me-md-0 {{ $isFirst ? 'active' : '' }}"
                    id="tab-{{ $sub_category->id }}"
                    data-bs-toggle="tab"
                    data-bs-target="#pane-{{ $sub_category->id }}"
                    type="button"
                    role="tab"
                    aria-controls="pane-{{ $sub_category->id }}"
    ...
    <div class="col-12 col-md-9 tab-content-container">
      <div class="tab-content" id="subcategory-content">
        @php $isFirst = true; @endphp
        @foreach($productSubCategories as $sub_category)
          @if($sub_category->category_id == 2)
            <div class="tab-pane fade {{ $isFirst ? 'show active' : '' }}"
                 id="pane-{{ $sub_category->id }}"
                 role="tabpanel"
                 aria-labelledby="tab-{{ $sub_category->id }}">
